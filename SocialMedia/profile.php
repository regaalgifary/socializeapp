...
$followers_result = $mysqli->query("SELECT COUNT(*) as total FROM followers WHERE id_user = $profile_id");
$followers = $followers_result->fetch_assoc()['total'];

$following_result = $mysqli->query("SELECT COUNT(*) as total FROM followers WHERE id_follower = $profile_id");
$following = $following_result->fetch_assoc()['total'];
...
.sidebar-left {
  ...
  bottom: 0;
  overflow-y: auto;
  z-index: 1050; /* Menjamin sidebar tetap di atas konten lainnya */
}
}
...
<div class="d-flex">
  ...
  <div class="bg-white rounded shadow-sm mb-3">
    ...
    <div class="text-center mt-n5">
      ...
      <?php else: ?>
        <form method="POST" action="follow_action.php" class="d-inline">
          <input type="hidden" name="following_id" value="<?= $profile_id ?>">
          <?php if ($is_following): ?>
            <button type="submit" name="action" value="unfollow" class="btn btn-outline-danger btn-sm mb-3">Unfollow</button>
          <?php else: ?>
            <button type="submit" name="action" value="follow" class="btn btn-outline-primary btn-sm mb-3">Follow</button>
          <?php endif; ?>
        </form>
      <?php endif; ?>
    </div>
